...
In Ty, one can tag not only pages, but also usesr. A user tag functions
as a user badge: the badge (tag) title can be configured to appear
next to the names of people with that badge, for example in their posts,
or in the about-user dialog.

Why 1) user badges, instead of 2) user titles or group member titles?

- Because of in-the-distant-future gamification capabilities:
  Auto granting badges to people who have done nice things. It can sometimes
  be nice if such badges appear next to the person's username.
  E.g. a badge with reputation points/karma, or maybe "Helpful Member"
  for someone who has answered many questions.

  But if there's both user badges, and also user titles / group-member titles,
...
- Settings for styling titles would need to be duplicated between pats_t and tags_t?
  E.g. title background color or icons? (Or, there'd be a new table with
  title styles, which would also be a bit extra complexity.)

  Or, if a title should appear only in some categories (like one's user flairs
  in Reddit appear only in some subreddits) — then, settings for that would
  need to get duplicated between pats_t and tags_t, _or_ there'd be an extra table?

- Maybe maybe a security issue, if using groups for titles? A mod might want to
  give someone a title, and so adds hen to a group with that user title.
  But groups can have security settings, and 1) maybe the mod didn't remember
  all security settings associated with that group — and by adding the user,
  accidentally granted unintended permissions. Or, 2) if showing
  a confirmation dialog about any new permissions, maybe the mod then realized
  hen couldn't place the person in that group, and now instead had to type
  the title manually on the user's profile page, or create another group
  with the same title but no additonal permissions.

  Badges, though, would be purely informational; no additonal permissions.
  And the mod could just add the badge to the person directly.
